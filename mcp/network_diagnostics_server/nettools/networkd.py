...
async def get_network_status_impl(interface: str, output_json=False) -> Dict[str, Any]:
    try:
        ...
        result = subprocess.run(["networkctl", f"--json={"short" if output_json else "off"}",  "status", interface], check=True, text=True, capture_output=True)
        ...
        logger.error(f"Error running networkctl status for interface {interface}: {str(e.output)}")
        return {"success": False, "message": str(e.output) + ". Consider if the interface exists or if the interface was spelled correctly. Case sensitivity matters."}
...
async def get_network_config_file_impl(interface: str) -> str:
    ...
    network_file_dropin_contents = [{ "file": f, "contents": pathlib.Path(f).read() } for f in network_file_dropins]
    network_file_contents = None

    if network_file:
        network_file_contents = { "file": network_file, "contents": pathlib.Path(network_file).read() }
...
async def reload_network_impl():
    ...
    try:
        ...
        return {
            "success": False,
            "message": f"Failed to reload systemd-networkd: {e}"
        }