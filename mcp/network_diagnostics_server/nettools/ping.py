...
This module implements the ping diagnostic tool for the Fleet Management system.
"""
...
async def run_ping_impl(host: str, count: int = 4, ctx = None) -> Dict[str, Any]:
    ...
    try:
        ...
        packets_stats = re.search(r'(\d+) packets transmitted, (\d+) packets received, (\d+)% packet loss', output)
        if packets_stats:
        ...
        timing_stats = re.search(r'min/avg/max = ([\d.]+)/([\d.]+)/([\d.]+)', output)
        if timing_stats:
